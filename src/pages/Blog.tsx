...
const fallbackDescriptions: Record<string, string> = {
  "what-is-an-intervention":
    "Start here if your family is still trying to understand what an intervention actually is and what a professionally guided process looks like.",
  "signs-loved-one-needs-intervention":
    "If you keep asking whether things are really serious enough yet, this article helps you stop minimizing what is already happening.",
  "firing-the-banker-and-the-janitor":
    "One of Matt's core ideas. This explains why families get stuck doing jobs addiction should no longer be allowed to outsource to them.",
};
...
const Blog = () => {
  ...
  const getPostCategory = (post: { category?: string | null }) =>
    post.category || "Intervention";
  ...
  const categories = useMemo(() => {
    ...
    return [
      "All",
      ...Array.from(
        new Set(blogPosts.map((post) => getPostCategory(post)).filter(Boolean)),
      ),
    ];
  }, [blogPosts]);
        ...
        <div className="container px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-lg text-muted-foreground">
              Resources, insights, and guidance for families navigating
                ...
                <section className="mb-10 rounded-3xl border border-primary/20 bg-primary/5 p-6 md:p-8">
                  <div className="max-w-3xl mb-6">
                    ...
                    <p className="text-muted-foreground leading-relaxed">
                      If your family is in crisis or just now realizing this
                      problem is bigger than a few bad incidents, start with
                  ...
                  <div className="grid md:grid-cols-3 gap-4 md:gap-6">
                    {featuredPosts.map((post) => (
                      <Link
                        key={post.id}
                        to={`/blog/${post.slug}`}
                        className="rounded-2xl border border-primary/15 bg-background p-6 hover:border-primary/40 transition-colors group"
                      >
                        ...
                        <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                          {fallbackDescriptions[post.slug] || post.excerpt}
                        </p>
                        <span className="text-primary text-sm font-medium inline-flex items-center gap-1">
                          Read this first <ArrowRight className="w-4 h-4" />
                        </span>
                      </Link>
                    ))}
                  </div>
