...
**Not published or remotely deployed.** Backend access to Freedom's project `rizfkjgwhcpwiryyqejx` returned HTTP 403. The exact Lovable project `dcb1167a-1709-40df-a59a-3f779d6703f9` requires sign-in. Keep this branch off main until backend deployment and admin QA are complete; replacing the existing tab before its RPC exists would leave it unavailable.

## Contract

The replacement Attribution tab reads only `get_central_attribution`. SQL independently requires the existing `is_strict_admin()` helper (server Auth identity, admin role and exact approved email), not user-editable metadata. No Google credentials or service-role keys enter React. Tables have RLS, admin SELECT only, service-only ingestion, constrained metric/brand/channel values and no free-form payload column.
...
6. Push/merge verified source and use Lovable **Share → Publish** on the exact project. Verify the authenticated `/admin` Attribution tab, phone business filters, 7/28/custom date windows, coverage, and desktop/mobile/keyboard behavior live.
...
- Historical website consent/URL minimization remains a separate release gate. Direct admin loads now skip GA4/Clarity initialization and route/custom tracking calls; existing trackers already loaded before an SPA transition are not fully isolated by those guards. Do not claim a complete sitewide privacy fix.
...
- Full prerender build exceeded the execution window and its browser was closed. Client production build and TypeScript passed; full prerender remains an explicit release gate.
